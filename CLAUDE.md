...
- **Human/Privacy Gate**: `human_detection_confidence` (0.3) - MegaDetector person-category confidence that, alone or together with a `homo` taxonomy segment in the SpeciesNet ensemble prediction, classifies a burst as `DetectionStatus.HUMAN`; evaluated *before* the animal branch, so a frame with both a person and a confident animal still routes to HUMAN. `suppress_human_alerts` (true) - HUMAN-status detections are still species-ID'd and DB-logged, but no Telegram notification is sent (not REVIEW-tagged, suppressed entirely). `human_retention_hours` (48) - saved burst photos for HUMAN-status detections are purged this many hours after capture; the DB row is kept as a metadata-only record. `human_retention_proximity_seconds` (default 240.0, exp #11 extension 2026-08-01) extends that purge **symmetrically in time** to review-class (NO_ANIMAL/UNCLASSIFIABLE) bursts that sit within this many seconds of a HUMAN-status detection *in either direction*: a burst that really contains a person but was misclassified `no_animal` (the leak class the Human-Proximity Gate below exists for) would otherwise keep recognizable frames on disk for the full `max_images` rotation (days), defeating the 48h human-photo policy. The purge runs ≥48h after capture, so unlike the notification gates it may look forward as well as backward. **Exp #35 extension (2026-09-23, commit `0bc9ec9`)**: a row already carrying `human_proximity_muted = 1` is purge-eligible *unconditionally*, with no time test — the mute gate decides on window OR density OR demoted-band, so a burst it muted can sit arbitrarily far from any single HUMAN row (burst 5444 was muted by the density condition at 310s and holds a recognizable person at close range, 70s outside the ±240s purge window). Purge eligibility follows the gate's own verdict; the ±window test is unchanged for *unmuted* review-class rows, which is the leading-edge case it exists for (a burst before a visit's first HUMAN burst necessarily has the flag False). Strictly a superset — 12 rows corpus-wide ever fell in this gap, ~1.4/month. `window_seconds <= 0` still short-circuits the whole extension, so the rollback lever below stays complete. Implemented as `DatabaseManager.get_human_adjacent_review_detections(cutoff, window_seconds)` (adjacency matched in Python over two indexed queries — a correlated SQL `EXISTS`/`strftime` subquery measured 3.9s per call on a 4k-row DB and `purge_human_bursts()` runs after every detection) plus a second sweep in `StorageManager.purge_human_bursts()`. DB rows are kept (metadata-only) exactly as for HUMAN rows; only image files are deleted. Rollback lever: `PERFORMANCE_HUMAN_RETENTION_PROXIMITY_SECONDS=0` + service restart.
- **Blur Gate**: `min_sharpness_threshold` (11.0) - a burst whose best frame scores below this is no longer silently discarded. It still gets a real image path + `sharpness_info`, flows through species ID, and is always DB-logged. The notification layer then decides: an animal found in a below-floor burst still alerts (with `below_sharpness_floor` noted); a below-floor burst with a review-class status (NO_ANIMAL/UNCLASSIFIABLE, no animal found) is DB-logged but *not* sent to Telegram, so REVIEW-channel volume doesn't rise. The human-privacy gate takes precedence over the blur mute (a blurry human burst is suppressed as HUMAN, not as blur); the blur mute in turn takes precedence over the Confident-Blank Mute Gate below (a below-floor AND blank-confident review burst is suppressed as blur, not blank-confidence).
- **Confident-Blank Mute Gate**: `blank_confidence_mute_threshold` (default 0.92, env `PERFORMANCE_BLANK_CONFIDENCE_MUTE_THRESHOLD`) - exp #29 (2026-09-17), a fourth mute path for review-class (NO_ANIMAL/UNCLASSIFIABLE) bursts, positioned between the Blur Gate and the Scene-Unchanged Gate. Fires when the species classifier's RAW top-1 prediction (`top_species_raw`, before geofence/rollup — see the observability-columns bullet below) is SpeciesNet's fully-generic "blank" (empty frame) verdict (`utils.is_blank_label`: last `;`-segment `blank`, every taxonomy segment empty, sentinel segments counted as empty per the exp #23 lesson) at or above this confidence. Measured over all 182 review-class rows corpus-wide with a blank raw top-1 and a recorded `top_species_score`: the 5 rows ever human-labelled `animal`/`animal_wrong_id` score 0.6431-0.8475 (ceiling **0.8475**); 42 human-confirmed false positives score median 0.9219, max 0.9825. The default 0.92 mutes 52/182 (29%) of blank review-class bursts corpus-wide with **zero** animal-labelled and zero person-labelled rows caught (highest person-labelled row: 0.9116) - a margin of 0.0725 over the animal ceiling, 3.6x the protocol's pre-registered `max(animal)+0.02` rule. This is the "different discriminator" experiments #16/#17 concluded was needed: the Scene-Unchanged Gate's pixel-similarity discriminator does NOT separate animal from FP on this corpus (animal rows 0.7886-0.9202 sit entirely inside the FP range 0.5335-0.9675), but the classifier's own blank-confidence does. Computed in `WildlifeSystem.process_detection` immediately after `top_species_raw`/`top_species_score`, persisted as the `blank_confidence_muted` BOOLEAN column - `True`/`False` for every review-class row the gate evaluated, `NULL` when the status isn't review-class or the gate is disabled (same "gate didn't apply" convention as `scene_gate_muted`), set directly on the initial INSERT (no follow-up UPDATE needed). The burst is still species-ID'd and DB-logged either way; only the Telegram send is skipped, with a `[BLANK-CONF]` log line. Fails open to `False` (never mutes) on any error; `0.0` is a special-cased DISABLE, not "mute everything" (a literal `score >= 0.0` would be the dangerous direction) - the rollback lever. The autonomous loop is confined to a tighter range, `loop.guardrails.BOUNDS["PERFORMANCE_BLANK_CONFIDENCE_MUTE_THRESHOLD"] = (0.87, 1.0)` = `max(animal-labelled blank score)=0.8475 + 0.02`, so it can never deploy a value at or below the measured animal ceiling; only a human editing the env file directly can set `0.0`. **Precedence**: Human/Privacy Gate > Human-Proximity Gate > Blur Gate > **Confident-Blank Mute Gate** > Scene Gate > Review Sampling Gate > Deferred REVIEW Send Gate - a below-floor OR HUMAN OR human-proximity-muted burst is never also evaluated for blank-confidence, and a blank-confidence-muted burst produces exactly one suppression log, never a second `[SCENE-GATE]`/`[REVIEW-SAMPLE]` one. Rollback lever: `PERFORMANCE_BLANK_CONFIDENCE_MUTE_THRESHOLD=0` + service restart, or `git revert 3c2856c`.
- **Scene-Unchanged Gate**: `scene_gate_enabled` (code default in `config.py` is still **`False`**) - a second, independent mute path for review-class (NO_ANIMAL/UNCLASSIFIABLE) bursts: `scene_gate.py::compute_scene_similarity` compares the burst's best frame (grayscale, downsized, mean/std-normalized to resist exposure/AE shifts) against a rolling `SceneReferenceSet` of the last `scene_gate_ref_count` (3) review-class best frames within `scene_gate_ref_max_age_hours` (6h); a similarity >= `scene_gate_similarity_threshold` (0.97) sets `scene_gate_muted=True` - the burst is still species-ID'd and DB-logged (`scene_similarity` REAL, `scene_gate_muted` BOOLEAN; both NULL when the gate is disabled, and `scene_gate_muted` alone is NULL when the status isn't review-class; `scene_gate_muted` is False (not NULL) when evaluated against an empty/failed reference set). **Since 2026-09-04 (commit below, backlog #17) `scene_similarity` is measured and DB-logged for EVERY status, not only review-class** - pure observability, no routing effect: the DECISION (`scene_gate_muted`, and the reference-set `add()`) is still review-class-only, so IDENTIFIED/HUMAN/ERROR bursts are compared against the reference set but can never be muted by it and never become references themselves. Rationale: the gate's threshold is unvalidatable while the animal bucket is empty, and IDENTIFIED rows are the only large supply of animal-containing bursts; frames roll off disk within ~`max_images` bursts, so the number cannot be reconstructed retroactively but no Telegram notification is sent, same pattern as the Blur Gate. Fails open (never mutes) if disabled, if the reference set is empty, or on any frame-read/comparison error - `compute_scene_similarity` never raises. **Precedence**: Human/Privacy Gate > Human-Proximity Gate > Blur Gate > Confident-Blank Mute Gate > Scene Gate (> Review Sampling Gate, see below) - a below-floor or HUMAN or human-proximity-muted or blank-confidence-muted burst gets exactly one suppression log (the earliest-precedence gate wins); `scene_similarity`/`scene_gate_muted` are still computed and DB-logged for every review-class burst regardless of blur/proximity/blank-confidence status. The reference set is seeded from the DB at `WildlifeSystem` startup (`DatabaseManager.get_recent_review_detections`) and updated on every subsequent review-class detection; HUMAN-status frames are never added as references. **Enabled by human override since 2026-07-26**: the gate shipped disabled (Task 5, 2026-07-17) because `scripts/validate_scene_gate.py`'s offline replay found zero human `animal`/`animal_wrong_id`-labeled review-class rows with a frame still on disk - all 17 such rows corpus-wide predate the ~100-burst image retention window - so per the spec's FN-veto acceptance rule no threshold could be validated from evidence. On 2026-07-26 Daniel explicitly overrode that HOLD as an accepted-risk decision to cut REVIEW volume (paired with the Review Sampling Gate below), deploying `PERFORMANCE_SCENE_GATE_ENABLED=true` at the placeholder threshold 0.97 despite the still-empty animal-labeled bucket - see `experiments/PROTOCOL.md` → "Scene-gate ownership" (SUPERSEDED 2026-07-26 block) and `experiments/runs/0009-review-volume-reduction.md`. The loop must not re-derive the threshold or disable the gate on the grounds that the animal bucket is empty - that condition is known and already ruled on. Rollback lever: `PERFORMANCE_SCENE_GATE_ENABLED=false` + service restart.
- **Review Sampling Gate**: `review_sample_rate` (default 0.25) - a sixth, last-precedence-before-deferral mute path for review-class (NO_ANIMAL/UNCLASSIFIABLE) bursts that survive the Human/Proximity/Blur/Confident-Blank/Scene gates above: only this fraction of them are actually sent to Telegram, to cut REVIEW-channel volume (deployed alongside the Scene-Unchanged Gate override, 2026-07-26 - see `experiments/runs/0009-review-volume-reduction.md`). The decision is deterministic per burst, not random per run: `wildlife_system._review_sample_fraction(detection_id)` hashes `detection_id` with sha256 into a stable `[0, 1)` fraction, and `is_review_sampled_out(detection_id, review_sample_rate)` mutes when that fraction `>= review_sample_rate` (so a given detection_id always lands on the same side of the gate). Fails open: a `None` detection_id (e.g. the DB write itself failed) always sends; `review_sample_rate >= 1.0` never samples out (the rollback lever); `<= 0.0` always samples out. Computed in `WildlifeSystem.process_detection` for every review-class row immediately after the DB INSERT (via a follow-up `DatabaseManager.update_review_sampled_out` UPDATE, since `detection_id` doesn't exist until the INSERT returns) and persisted as the `review_sampled_out` BOOLEAN column - `True`/`False` for every review-class row, `NULL` for non-review-class rows and rows logged before this gate existed. The burst is still species-ID'd and DB-logged either way; only the Telegram send is skipped, with a `[REVIEW-SAMPLE]` log line. **Precedence**: Human/Privacy Gate > Human-Proximity Gate > Blur Gate > Confident-Blank Mute Gate > Scene Gate > Review Sampling Gate - it is evaluated and logged last among the mute gates, so a burst any earlier gate would already suppress produces exactly one suppression log, never a redundant second one. `loop/metrics.py`'s `n_sampled_out` count (surfaced in the nightly report as "• Not sent (review sampling): N", same treatment as `n_cant_tell`) excludes these rows from the "Not yet labelled" backlog line, since they were never sent for a human to label - it does not affect `fp_rate` or the per-tier FP buckets, which remain label-conditioned as before. Rollback lever: `PERFORMANCE_REVIEW_SAMPLE_RATE=1.0` + service restart.
- **Animal-Proximity Review Exemption**: `animal_proximity_window_seconds` (default 180.0, exp #33 `animal-proximity-review-exemption`, 2026-09-20) - un-mutes rather than mutes: a review-class (NO_ANIMAL/UNCLASSIFIABLE) burst landing within this many seconds after the most recent **named-animal** IDENTIFIED detection is exempted from the Review Sampling Gate above ONLY, so it always sends as a REVIEW message instead of being sampled out. Modeled directly on the Human-Proximity Mute Gate below, inverted. Motivating case: burst 5388 (09:12:59) was correctly IDENTIFIED as `aves;;;;;bird`; burst 5389, the same blackbird 25s later, came back `unclassifiable` and was then sampled out by the Review Sampling Gate - a silent false negative that never reached Telegram. Measured corpus-wide: the Review Sampling Gate is the ONLY mute path that ever suppresses a review-class burst shortly after a named-animal IDENTIFIED burst (the human-proximity, blur, confident-blank and scene gates muted zero such rows, ever); six rows were sampled out within 180s of a named-animal identification, three human/tier-2-labelled `animal` (gaps 25s, 25s, 124s), the nearest `false_positive`-labelled row sitting at 206s - so 180s recovers every known animal with margin and stops short of the nearest known FP. `WildlifeSystem` tracks `self._last_animal_detection_at` in memory (seeded at startup from `DatabaseManager.get_last_animal_detection_time()`, updated whenever `process_detection` classifies a burst `DetectionStatus.IDENTIFIED` with a label satisfying the new `utils.is_named_animal_label` - true when the label is NOT `is_unnamed_animal_label`, NOT `is_blank_label`, and carries no `homo` taxonomy segment, with at least one non-empty, non-sentinel taxonomy/common-name segment); the exemption fires when `0 <= (burst_time - last_animal).total_seconds() <= animal_proximity_window_seconds`, logging `[ANIMAL-PROXIMITY]`. **No new DB column**: `review_sampled_out` is persisted exactly as before (`False` when exempted) - the exemption is reconstructable offline since `_review_sample_fraction(detection_id)` is deterministic. **Precedence is unaffected, by construction, not by an extra check**: the exemption is evaluated in `process_detection` immediately before `is_review_sampled_out` is consulted, strictly after every earlier-precedence gate (Human/Privacy, Human-Proximity, Blur, Confident-Blank, Scene) has already been computed and persisted to its own independent flag; `_process_and_notify_detection`'s `is_sampled_out_review` still reads `review_sampled_out` gated behind `not is_human and not is_human_proximity_review and not is_unnamed_animal_blank and not is_blurry_review and not is_blank_confidence_review and not is_scene_unchanged_review`, so a burst any earlier gate would already mute stays muted via that gate's own flag regardless of what this exemption did to `review_sampled_out` - the exemption can only ever turn a would-be `[REVIEW-SAMPLE]` suppression into a send, never override an earlier `[HUMAN-GATE]`/`[HUMAN-PROXIMITY]`/`[UNNAMED-BLANK]`/`[BLUR]`/`[BLANK-CONF]`/`[SCENE-GATE]` suppression. Fails open to today's (unexempted) sampling behaviour on any exception, a `None`/zero window, or no prior named-animal detection - never raises. Rollback lever: `PERFORMANCE_ANIMAL_PROXIMITY_WINDOW_SECONDS=0` + service restart (disables BOTH halves - see the forward half immediately below). **Leading-edge (forward) half (exp #39, 2026-09-25, commit 657a30c)**: the exemption above is *backward-looking by construction*, so it cannot save the LEADING edge of an animal visit - a review-class burst that the Review Sampling Gate mutes seconds BEFORE SpeciesNet names the same animal. Burst 5448 (07:36:43, `unclassifiable`) held a calico cat at extreme close range and was suppressed by a `[REVIEW-SAMPLE]` coin flip; burst 5449, the same cat, was correctly IDENTIFIED as `mammalia;carnivora;;;;carnivorous mammal` 37s later. This is the exact structural mirror of the human-side gap the Deferred REVIEW Send Gate already fixes, so it reuses that machinery: when `animal_proximity_window_seconds > 0`, a sampled-out review-class burst is no longer dropped on the spot but handed to `_schedule_deferred_review_send(..., require_animal_proximity=True)`. `_deferred_review_send` then runs two phases - Phase 1 sleeps `animal_proximity_window_seconds` and re-reads `self._last_animal_detection_at`: if no named-animal IDENTIFIED detection landed strictly within `(burst_time, burst_time + window]` it logs the same `[REVIEW-SAMPLE]` line the immediate path would have and returns without sending (`review_sampled_out` stays `True` - the common case, still silent on Telegram); if one did, it logs `[ANIMAL-DEFER]`, persists `update_review_sampled_out(detection_id, False)` and falls through to Phase 2 with only the REMAINING defer budget (`review_defer_seconds - animal_window`), so the unchanged cancel-on-human test still spans its full `review_defer_seconds` from the burst's own timestamp. Waking exactly at `burst_time + window` is what makes Phase 1's test trivially sound: `_last_animal_detection_at` is by definition the most recent named animal at or before that instant. **Privacy precedence is preserved** - a person arriving after the burst still wins over a recovered animal. **Fail-open direction is phase-dependent, deliberately**: an exception in Phase 1, before the animal decision, falls back to SUPPRESS (a sampled-out burst is already a deliberate drop; a bug must not manufacture notification volume), while any failure after the recovery decision falls open to SEND as everywhere else in that function; `asyncio.CancelledError` still propagates untouched so shutdown never fires a spurious send. Measured over the full corpus before shipping (484 `review_sampled_out=1` rows vs 175 `is_named_animal_label` IDENTIFIED rows): a 180s forward window un-mutes exactly **2 rows corpus-wide** - 5448 (37s, the cat) and 5359 (72s, tier-2 `false_positive`) - i.e. ~1 extra REVIEW message per two months to recover a confirmed animal, flat between 120s and 240s so 180s is not a knife-edge. Can only ever ADD a notification, never mute one, so FN cannot rise. No new config field, no new DB column, no new rollback lever: `PERFORMANCE_ANIMAL_PROXIMITY_WINDOW_SECONDS=0` disables both halves at once, or `git revert 657a30c`.
- **Human-Proximity Mute Gate**: `human_proximity_window_seconds` (default 120.0) - a second, independent mute path layered on top of the Human/Privacy Gate for review-class (NO_ANIMAL/UNCLASSIFIABLE) bursts: MegaDetector scores extreme close-up / motion-blurred partial human bodies at only ~0.02-0.15 person confidence, too low to trip the Human/Privacy Gate itself, so such bursts slip through as `no_animal` and leak a recognizable person to REVIEW. Three such leaks occurred on 2026-07-27 (detection ids 3544, 3553, 3554), each 76-108s after a correctly-classified `human`-status burst. `WildlifeSystem` tracks `self._last_human_detection_at` in memory (seeded at startup from `DatabaseManager.get_last_human_detection_time()`, updated every time a detection is classified `DetectionStatus.HUMAN`); a review-class burst is muted when it lands within `human_proximity_window_seconds` after that timestamp (`0 <= (burst_time - last_human).total_seconds() <= window`). Validated against all 12 human-labelled `animal`/`animal_wrong_id` review-class rows since the human gate went live (2026-07-08): the closest is 329s from a preceding human-status burst, so the 120s default window costs zero known false negatives. The decision is persisted as the `human_proximity_muted` BOOLEAN column - `True`/`False` for every review-class row, `NULL` for non-review-class rows (same convention as `scene_gate_muted`) - written on the same DB write as the other observability fields (no follow-up UPDATE needed, unlike `review_sampled_out`, since it doesn't depend on this row's own id). The burst is still species-ID'd and DB-logged either way; only the Telegram send is skipped, with a `[HUMAN-PROXIMITY]` log line. Fails open: any error computing proximity, a `0.0` window, or no prior human detection all result in no mute, never a blocked notification. **Density condition (exp #11 mechanism extension, 2026-07-28)**: a second, OR-ed condition on the SAME gate — tonight's adjudication found recognizable-person review-class bursts that fell OUTSIDE the 120s window (gaps of 432s and 732s past the last human burst) during a long gardening session, so a single "time since last human" window can't catch a garden that stays occupied longer than the window. `human_density_window_seconds` (default 1800.0) and `human_density_count` (default 8) add a "garden is occupied" test: `WildlifeSystem` also tracks `self._recent_human_detection_times` (a list of HUMAN-status timestamps, seeded at startup from `DatabaseManager.get_recent_human_detection_times(since)`, appended to on every HUMAN classification, pruned to `human_density_window_seconds` before every use via `_count_recent_human_detections` so it can't grow unbounded); a review-class burst is also muted when at least `human_density_count` HUMAN-status detections occurred in `[burst_time - human_density_window_seconds, burst_time]`. Same `human_proximity_muted` DB column (no new column), same `[HUMAN-PROXIMITY]` log line (now noting `window` vs `density` in the message text), same precedence. `human_density_count=0` disables the density condition (rollback lever) while leaving the window condition untouched; fails open identically (any exception, or `window=0` AND `count=0`, or no prior human detections → no mute). **Precedence**: Human/Privacy Gate > Human-Proximity Gate (window OR density) > Blur Gate > Confident-Blank Mute Gate > Scene Gate > Review Sampling Gate - evaluated right after the Human/Privacy Gate so a HUMAN-status burst produces exactly one suppression log ([HUMAN-GATE], not [HUMAN-PROXIMITY]), and a burst this gate mutes never also produces a [BLUR]/[BLANK-CONF]/[SCENE-GATE]/[REVIEW-SAMPLE] log. Rollback levers: `PERFORMANCE_HUMAN_PROXIMITY_WINDOW_SECONDS=0` (disables the window condition) and/or `PERFORMANCE_HUMAN_DENSITY_COUNT=0` (disables the density condition) + service restart. **Unnamed-animal scope widening (exp #26, 2026-09-14, commit `80d0c00`)**: this gate is no longer review-class-only. SpeciesNet's fully-generic rollup `<uuid>;;;;;;animal` ("an animal is there but I cannot name it" — its ensemble confidence is literally MegaDetector's box confidence, no classifier verdict attached) routes to `DetectionStatus.IDENTIFIED`, so it produced a MAIN-channel species alert that bypassed *every* review-class mute path (blur/scene/sampling/deferral/human-proximity all test `is_review_detection`). A person at extreme close range is exactly the input that produces it: bursts 5222 and 5270 on 2026-09-14 were close-ups of clothing and a leg, 30s and 82s after HUMAN-status bursts, both delivered to MAIN as "animal detected". Re-routing the label was FN-vetoed (18 of the 78 rows with this shape corpus-wide are human-labelled `animal`/`animal_wrong_id`), so the gate's scope was widened instead: `process_detection` now evaluates the window-OR-density test for review-class bursts **or** IDENTIFIED bursts whose label satisfies `utils.is_unnamed_animal_label` (last `;`-segment `animal` and every taxonomy segment empty, with `no cv result`/`blank` counted as empty per exp #23's sentinel lesson; `aves;;;;;bird` and any named species do NOT match). Measured over all 78 such rows: mutes exactly 4, zero of them human-labelled animals. `human_proximity_muted` is therefore now `True`/`False` (not `NULL`) on unnamed-animal IDENTIFIED rows too — the NULL-for-non-review-class convention now means "this gate didn't apply", not "the status isn't review-class". A muted burst is suppressed entirely with exactly one `[HUMAN-PROXIMITY]` log (Blur/Scene/Sampling still require review-class, so no second log is possible), and `DatabaseManager.get_human_adjacent_review_detections` also returns `identified` rows with `human_proximity_muted = 1` so their frames are purged on the 48h human-retention policy rather than the full `max_images` rotation. Rollback: `git revert 80d0c00` + restart. **Demoted-band window widening (exp #27, 2026-09-15, commit `3752a7b`)**: `human_demoted_person_floor` (default 0.3) and `human_demoted_window_seconds` (default 1800.0) make the *window* condition's look-back band-conditional on the burst's OWN `person_confidence`, rather than adding a third condition. Exp #14 raised `SPECIES_HUMAN_DETECTION_CONFIDENCE` 0.3→0.5 on the explicit promise that the layered proximity/density gates would still mute the real people scoring in the demoted 0.3-0.5 band; burst 5305 on 2026-09-15 (a close-up of clothing, `person_confidence` 0.436) is the first measured failure of that promise — 480s after the last HUMAN burst (window 240s) with only 5 HUMAN bursts in the trailing 1800s (density needs 8), so both conditions missed it and only the 50% Review Sampling Gate coin flip kept it out of Telegram. Neither env lever reaches it without a known false negative (a global window ≥480s mutes id 1838 at 329s; a density count ≤5 mutes id 2011 at density 5), but `person_confidence` separates the populations cleanly — the max over all 20 human-labelled `animal`/`animal_wrong_id` rows corpus-wide is 0.0789 vs 5305's 0.436. So when a burst's own `person_confidence >= human_demoted_person_floor` the window condition uses `max(human_proximity_window_seconds, human_demoted_window_seconds)` (1800s, deliberately reusing the density condition's existing "garden is occupied" horizon rather than a third time constant) instead of `human_proximity_window_seconds`. Scope, precedence, the density condition, the `human_proximity_muted` column and the single `[HUMAN-PROXIMITY]` log are all unchanged; the log names `demoted-band window` as the reason only when the widening is what actually caused the mute. Measured over all 3103 review-class + unnamed-animal rows: only 19 carry `person_confidence >= 0.3` at all, and this mutes 4 of them (5305 the person; 5090 an adjudicated empty-garden phantom; 4765/4782 frames rolled off disk), zero human-labelled animals — ~1.3 mutes/month. Fails open on a missing/`None` `person_confidence`, on `human_demoted_window_seconds == 0.0`, and on any exception. Rollback lever: `PERFORMANCE_HUMAN_DEMOTED_WINDOW_SECONDS=0` + service restart (restores the flat 240s window, density condition untouched), or `git revert 3752a7b`.
- **Deferred REVIEW Send Gate (cancel-on-human)**: `review_defer_seconds` (default 240.0, exp #11 extension 2026-08-01) - the Human-Proximity Gate above is *backward-looking by construction*, so it can never mute the **leading edge** of a human visit: the first burst of a visit is often a close-up motion smear classified `no_animal`, with the visit's first HUMAN-status burst arriving seconds *later*. Three such bursts reached REVIEW (3829 on 07-29 at 75s, 3867 on 07-30 at 51s, 3909 on 07-31 at 81s before the first HUMAN burst); 3909's saved frames contain a clearly recognizable face. Fix: a review-class burst that survives every gate above does **not** send immediately — `WildlifeSystem._schedule_deferred_review_send` builds the annotated image synchronously (it depends on `self.last_motion_frame`, which mutates on the next loop iteration) and then hands a background asyncio task everything the send needs; `_deferred_review_send` sleeps `review_defer_seconds` and cancels the Telegram send if a HUMAN-status detection landed in the meantime (`burst_time < self._last_human_detection_at <= burst_time + review_defer_seconds`), logging `[REVIEW-DEFER]` and persisting the mute via `DatabaseManager.update_human_proximity_muted` (same column as a backward mute — a follow-up UPDATE, since the decision isn't known at INSERT time). The main detection loop never awaits the deferral; pending tasks are tracked in `self._pending_review_tasks` and cancelled in `run()`'s shutdown path. Only review-class sends are deferred — MAIN/animal notifications are never delayed. Fails open: any exception sends the notification (`asyncio.CancelledError` is a `BaseException` and deliberately propagates, so a shutdown cancel never triggers a spurious send). FN cost measured, not assumed: of the 12 human-labelled `animal`/`animal_wrong_id` review-class rows since the human gate went live (2026-07-08), the closest sits 1846s before the next HUMAN burst - 7.7x the 240s window - so the gate costs zero known false negatives; it cancels ~18% of otherwise-sent review-class bursts. **Precedence**: evaluated last, after Human/Privacy > Human-Proximity > Blur > Confident-Blank Mute Gate > Scene > Review Sampling, as the final step before an actual review-class send. Rollback lever: `PERFORMANCE_REVIEW_DEFER_SECONDS=0` + service restart (reviews then send immediately, as before).
...
- **"Best guess" caption line**: when the ensemble's final species label is a generic rollup (blank/empty genus or species taxonomy segments, e.g. `aves;;;;;bird` or `;;;;;;animal`) but the classifier's raw top-1 prediction (`metadata['top_classifier_prediction']`) names something more specific and non-generic, the Telegram caption appends a line `Best guess: <common name> (NN%)` (built by `WildlifeSystem._best_guess_line`, using `utils.extract_common_name` to pull the last non-empty semicolon-delimited segment). Shown even at low confidence — that's the point. Wrapped defensively so a formatting error never blocks the notification; a non-dict/malformed `top_classifier_prediction` (legacy classifier shape) degrades to no caption line and NULL DB columns rather than crashing. **Geofence preference (exp #28, 2026-09-16, commit `838e5f6`)**: the raw top-1 is ungeofenced and pre-rollup, so on the rare bursts that really contain an animal it names either a species that cannot occur in the configured region or a tautology of the verdict itself — the three Eurasian blackbird bursts on 2026-09-16 (5307/5308/5309, human-labelled `animal`) were captioned `Best guess: Blue whistling-thrush (42%)` (a Himalayan species) and twice `Best guess: Bird (NN%)`. `_best_guess_line` now prefers `metadata['best_geofenced_species']` — the first species-level candidate in the classifier's top-k (`species.return_top_k`, 5) that SpeciesNet's own geofence map allows in `country_code`/`admin1_region`, computed on every identification by `SpeciesIdentifier._find_best_geofenced_species` and previously stored in metadata but never displayed; it held `common blackbird` on all three of those bursts (0.062/0.029/0.032, vs the impossible congener at 0.422). The raw top-1 remains the fallback when no in-region candidate exists (unchanged behaviour there), and a guess whose common name merely repeats the ensemble's own rollup name is now suppressed. Note the score inversion this exposes: the classifier is not region-aware, so raw score is the wrong selection rule for this line — the geographically correct answer can sit an order of magnitude below an impossible one. Caption text only: no routing, no mute, no DB column, no notification created or suppressed. Rollback: `git revert 838e5f6` + restart.
...
- Performance: `PERFORMANCE_COOLDOWN`, `PERFORMANCE_MAX_IMAGES` (burst image retention; default 300, raised from 100 on 2026-07-27 — see the Storage bullet above), `PERFORMANCE_SEND_ANNOTATED_IMAGE` (debug: send motion overlay alongside original, default false)
  ...
  `PERFORMANCE_HUMAN_DEMOTED_PERSON_FLOOR` (Human-Proximity Mute Gate's demoted-band widening: the `person_confidence` at or above which a burst's own sub-threshold person score widens the window condition's look-back, bounded `[0.0, 1.0]`; default 0.3), `PERFORMANCE_HUMAN_DEMOTED_WINDOW_SECONDS` (the widened look-back used for such bursts, bounded `[0.0, 7200.0]`; default 1800.0; `0` = widening disabled, the rollback lever)
  ...
  `PERFORMANCE_BLANK_CONFIDENCE_MUTE_THRESHOLD` (Confident-Blank Mute Gate: classifier raw-top-1-is-blank confidence at or above which a review-class burst is muted, config-level bounds `[0.0, 1.0]` but the autonomous loop's own range is tighter — `loop.guardrails.BOUNDS` bounds it to `[0.87, 1.0]`, above the measured animal ceiling of 0.8475; default 0.92; `0` = disabled, the rollback lever, settable only by a human editing the env file directly)
  `PERFORMANCE_ANIMAL_PROXIMITY_WINDOW_SECONDS` (Animal-Proximity Review Exemption: look-back window after the most recent named-animal IDENTIFIED detection during which review-class bursts are exempted from the Review Sampling Gate only, bounded `[0.0, 600.0]`; default 180.0; `0` = disabled, the rollback lever)
