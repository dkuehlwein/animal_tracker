...
- **Feedback labels (`src/feedback_protocol.py`, 2026-07-09 redesign)**: each Telegram detection notification carries a 5-button, 2-row inline keyboard, one tap per notification — row 1 "✅ Animal" / "🐦 Animal, wrong ID" / "👤 Human", row 2 "❌ Nothing there" / "🤷 Can't tell" — wired through `CODE_TO_LABEL` to the stored `detection_feedback.label`: `a`→`animal`, `wid`→`animal_wrong_id`, `p`→`person`, `fp`→`false_positive`, `ct`→`cant_tell`. Note the deliberate `person` vs `human` split: the button label is `person` (something/someone human-shaped was in frame) to avoid colliding with `human` as a *labeller tier* — `detection_feedback.source='human'` (vs `'tier2'`; tier-1 is derived straight from the `detections` row, not a `detection_feedback` entry) means "a person tapped this," independent of what they tapped, and is unrelated to `DetectionStatus.HUMAN`. `ws`→`wrong_species` is legacy: still accepted by `parse_callback_data` (so buttons on old channel messages keep working) and still a valid `VALID_FEEDBACK_LABELS` value in `database_manager.py`, but no longer shown on freshly built keyboards — it predates this redesign's split into `animal_wrong_id` (wrong animal species) and `person` (it was a human, not an animal), so its accumulated history is heterogeneous and conflates both cases; don't read it as a clean signal without checking the underlying frame. `cant_tell` ("unusable image," e.g. too blurry/dark to judge) wins reconciliation in `loop/ingest.py` like any other human label — a human "can't tell" verdict blocks tier-1/tier-2 auto-labels from backfilling that row — but `loop/metrics.py` excludes `cant_tell` rows from the fp_rate denominator (`labeled_triggers`) and from every per-tier FP bucket, surfacing them instead as a separate `n_cant_tell` count (not written to `daily.csv`); the nightly `loop/report.py` summary shows them as their own "• Can't tell (unusable image): N" line and excludes them from the "Not yet labelled" remainder. False negatives have no dedicated button — the signal is a human `animal`/`animal_wrong_id` label landing on a `no_animal`/`unclassifiable`-status row, read at query time via the `detections`⋈`detection_feedback` join (no code path computes this directly).
